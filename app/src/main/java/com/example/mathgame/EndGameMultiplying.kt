...
class EndGameMultiplying : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_end_game_multiplying)

        fun resetGameResults(){
            GameMultiplying.correctAnswers = 0
            GameMultiplying.incorrectAnswers = 0
            GameMultiplying.quitGameButtonTaps = 2
        }

        val finalCorrectAnswers: Int = GameMultiplying.correctAnswers
        val finalIncorrectAnswers: Int = GameMultiplying.incorrectAnswers
        ...
        val btnReplay: Button = findViewById(R.id.btn_multiplying_endgame_replay)
        val btnQuitGame: Button = findViewById(R.id.btn_multiplying_endgame_quit_game)

        findViewById<TextView>(R.id.et_multiplying_endgame_summary_card_correct_value).text = finalCorrectAnswers.toString()
        findViewById<TextView>(R.id.et_multiplying_endgame_summary_card_incorrect_value).text = finalIncorrectAnswers.toString()
        findViewById<TextView>(R.id.et_multiplying_endgame_final_percentage_value).text = finalPercentage.toString()

        val finalScore = findViewById<TextView>(R.id.et_multiplying_endgame_summary_final_score_value)
        if (finalCorrectAnswers+finalIncorrectAnswers == 0) {
            finalScore.text = ""
